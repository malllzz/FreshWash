...
router.post('/', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await db.execute(
      ...
      [orderCode, insertedId]
    );

    res.status(201).json({ message: 'Reservasi berhasil', order_id: insertedId, order_code: orderCode });
...
router.get('/services', async (req, res) => {
  try {
    const [rows] = await db.execute('SELECT service_id, name FROM services');
    res.json(rows);
  } catch (err) {
